...
authRouter.post("/otp/send", otpSendLimiter, async (req, res, next) => {
	try {
		...
		if (!phone)
			return res
				.status(400)
				.json({ message: "شماره موبایل نامعتبر است." });
...
authRouter.post("/otp/verify", otpVerifyLimiter, async (req, res, next) => {
	try {
		...
		if (!otp || otp.attempts >= 5)
			return res
				.status(400)
				.json({ message: "کد تایید اشتباه یا منقضی است." });
...
authRouter.patch("/me", async (req, res, next) => {
	try {
		...
		if (
			...
			return res.status(401).json({ message: "وارد نشده‌ای." });
		next(error);
	}
});
...
authRouter.post("/admin/login", adminLoginLimiter, async (req, res, next) => {
	try {
		...
		if (!admin || !admin.isActive)
			return res
				.status(401)
				.json({ message: "اطلاعات ورود اشتباه است." });
