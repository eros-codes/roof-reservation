...
function chairPositions(table, visual) {
	...
	if (layout === 'three-sides') {
		return [
			{ x: 0, y: -gapY, rotation: 180 },
			{ x: -gapX, y: h * 0.08, rotation: 90 },
			{ x: gapX, y: h * 0.08, rotation: -90 },
		];
	}
	if (layout === 'rect-4-sides') {
		return [
			{ x: -gapX, y: -h * 0.23, rotation: 90 },
			{ x: -gapX, y: h * 0.23, rotation: 90 },
			{ x: gapX, y: -h * 0.23, rotation: -90 },
			{ x: gapX, y: h * 0.23, rotation: -90 },
		];
	...
	if (layout === 'rect-6') {
		return [-0.32, 0, 0.32].flatMap((ratio) => [
...
function createTableTop(table, visual) {
	...
	if (visual.seatLayout === 'sofa-bottom') {
		group.append(svgEl('rect', { class: 'sofa-capacity', x: -w * 0.34, y: h / 2 + 8, width: w * 0.68, height: 8, rx: 4 }));
	}
	if (visual.seatLayout === 'sofa-right') {
		group.append(svgEl('rect', { class: 'sofa-capacity', x: w / 2 + 8, y: -h * 0.32, width: 8, height: h * 0.64, rx: 4 }));
	}
...
function createLounge(table) {
	...
	const h = Number(table.height);
	group.append(
		createSofaModule({ x: -w * 0.78, y: 0, width: 31, rotation: 90 }),
		createSofaModule({ x: 0, y: -h * 0.9, width: 31, rotation: 180 }),
		createSofaModule({ x: w * 0.82, y: 0, width: 31, rotation: -90, double: true }),
	);
	return group;
}
